...
1. **Переменные окружения** (необязательно): `BALTIKA_HOST`, `BALTIKA_PORT`, `BALTIKA_DB`. Учётная запись суперпользователя PostgreSQL для скриптов развёртывания задаётся при вызове `psql`, а не через приложение.
